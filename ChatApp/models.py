...
# フレンド追加機能、検索したユーザーのIDを格納
    def addFreind(my_id, friend_id):
        try:
            conn = DB.getConnection()
            cur = conn.cursor()
            sql = "INSERT INTO friends (my_id, friend_id) VALUES (%s, %s);"
            cur.execute(sql, (my_id, friend_id))
            conn.commit()
        except Exception as e:
            print(e + 'が発生しています')
        finally:
...
# フレンド一覧機能、自分のuser_idに紐付けされているfriend_idを一覧で引き出す
    ...
    # tryブロックを使用して、データーベースへの接続と操作を試みる
        try:
            ...
            sql = "SELECT id, my_id, , friend_id, u.name, u.one_phrase FROM friends AS f INNER JOIN users AS u ON f.friend_id = u.id WHERE my_id = %s;"
